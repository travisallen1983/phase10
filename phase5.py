def phase_five(current_player, key):
    print("Phase Five is one run of 8")
    run_of_eight = []
    number_ending = ""
    for i in range(1,9):
        if i == 1:
            ...
            number_ending = "8th"
        run_of_eight_input = input("What is the {card} card of your run of 8? ".format(card=number_ending))
        if run_of_eight_input.upper() not in current_player.get(key):
            input("What is the {card} card of your run of 8? ".format(card=number_ending))
        run_of_eight_input = run_of_eight_input.upper()
        run_of_eight.append(run_of_eight_input)
    phase = run_of_eight
    print(phase)
