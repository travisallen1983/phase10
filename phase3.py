def phase_three(current_player, key):
    print("Phase Three is one set of four and 1 run of four")
    set_of_four = []
    run_of_four = []
    number_ending = ""
    for i in range(1,5):
        if i == 1:
            ...
            number_ending = "4th"
        set_of_four_input = input("What is the {card} card of your set of 4? ".format(card=number_ending))
        if set_of_four_input.upper() not in current_player.get(key):
            input("What is {card} card of your set of 4? ".format(card=number_ending))        
        set_of_four_input = set_of_four_input.upper()
        set_of_four.append(set_of_four_input)
    for i in range(1,5):
        if i == 1:
            number_ending = "1st"
        elif i == 2:
